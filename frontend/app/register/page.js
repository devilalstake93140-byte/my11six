    ...
    try {
      ...
      if (googleResult.success) {
        ...
        if (response.data.success) {
          ...
          // Save token using Cookies
          import('js-cookie').then((Cookies) => {
            Cookies.default.set('token', token, { expires: 7 });
          ...
          setError(response.data.message || 'Failed to create account with Google');
        }
      } else {
        setError(googleResult.error || 'Google sign-up failed');
      }
    } catch (err) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-[#1a1a1a] rounded-2xl border border-[#2a2a2a] p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 mt-2">Join and start winning today!</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex items-start gap-2">
              <input type="checkbox" required className="w-4 h-4 mt-1 rounded bg-[#0a0a0a] border-[#2a2a2a]" />
          ...
          <button
            ...
            className="w-full mt-4 py-3 bg-[#0a0a0a] border border-[#2a2a2a] rounded-lg text-white font-medium hover:border-amber-500/30 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
